...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
    ...
    if (!wtx.strTxComment.empty())
       strHTML += "<b>" + tr("Transaction comment") + ":</b><br>" + GUIUtil::HtmlEscape(wtx.strTxComment, true) + "<br>";



    //
    // Wallet Message: does not go in chain, free
    //
    ...
    for (int k = 0; k < wtx.vout.size(); ++k)
    {
        ...
        if (wtx.mapValue.count(cbuf) && !wtx.mapValue[cbuf].empty())
             strHTML += "<br><b>" + tr(cbuf) + ":</b> " +
	             GUIUtil::HtmlEscape(wtx.mapValue[cbuf], true) + "<br>";
    }
