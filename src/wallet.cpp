    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s SWIFT %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s SWIFT %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> >& vecSend,
				...
				int64_t& nFeeRet, int32_t& nChangePos, std::string strTxComment,
				unsigned int nProdTypeID, const CCoinControl* coinControl)
{
    int64_t nValue = 0;
    ...
    // product
    wtxNew.nProdTypeID = nProdTypeID;
    // transaction comment
...

    {
        ...
        CTxDB txdb("r");
        {
            if (wtxNew.nTime < CTXV2_LIVE) {
              nFeeRet = nTransactionFee;  // <-- 0
            } else {
              nFeeRet = MIN_TX_FEE;       // <-- reasonable
            }
            while (true)
            {
                wtxNew.vin.clear();
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> >& vecSend,
    ...
    bool rv = CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, nChangePos,
                                  "", SWIFT_NONE, coinControl);
    return rv;

}

bool CWallet::CreateTransaction(CScript scriptPubKey, int64_t nValue, std::string& sNarr,
                                CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet,
                                std::string strTxComment, unsigned int nProdTypeID,
                                const CCoinControl* coinControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    
    if ((sNarr.length() > 0) && ((wtxNew.nTime >= (CTXV2_LIVE + 600)) || fTestNet))
    {
    ...
    int nChangePos;
    bool rv = CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, nChangePos,
                                             strTxComment, nProdTypeID, coinControl);
...
string CWallet::SendMoney(CScript scriptPubKey, int64_t nValue, std::string& sNarr,
                          CWalletTx& wtxNew, bool fAskFee,
                          std::string strTxComment, int nProdTypeID)
{
    CReserveKey reservekey(this);
    int64_t nFeeRequired;
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64_t nValue,
                                       ...
                                       std::string strTxComment, int nProdTypeID)
{
    ...
    return SendMoney(scriptPubKey, nValue, sNarr, wtxNew, fAskFee, strTxComment, nProdTypeID);
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s SWIFT %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s SWIFT %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
